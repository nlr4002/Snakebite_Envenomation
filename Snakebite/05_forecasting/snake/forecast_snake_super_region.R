...
year_forecast_list <- c(2030, 2040, 2050)

#Pull in draws
dt <- as.data.table(read.csv(paste0(my_dir, "region_snakes_draws_dismod_years.csv")))
...
for (year_forecast in year_forecast_list){
    ...
    fits_f <- lapply(0:999, function(i) {
      #i <- 1 # dev
      df_tmp <- as.data.frame(dt_f)[, c("super_region_name", "year_id", "midage", paste0("draw_", i))]
      names(df_tmp)[4] <- "draw_i"
      lm(draw_i ~ year_id * super_region_name + bs(midage,knots = c(25,50,75)), data = df_tmp) #interaction term region and year, cubic spline on age (midage)
    })
    
    fits_m <- lapply(0:999, function(i) {
      #i <- 1 # dev
      df_tmp <- as.data.frame(dt_m)[, c("super_region_name", "year_id", "midage", paste0("draw_", i))]
      names(df_tmp)[4] <- "draw_i"
      lm(draw_i ~ year_id * super_region_name + bs(midage,knots = c(25,50,75)), data = df_tmp) #interaction term region and year, cubic spline on age (midage)
    })
    ...
    df_matrix_f <- unique(df_matrix[df_matrix$sex_id==2,])
    df_matrix_m <- unique(df_matrix[df_matrix$sex_id==1, ])
    ...
    ## Save output
    output_path_reg <- paste0(output_dir, "output/")
    write.xlsx(final_dt, paste0(output_path_reg, year_forecast, "_snake_forecast.xlsx"))
}



## Final combined data
df1 <- read.xlsx(paste0(output_dir, "output/2020_snake_forecast.xlsx"))
df2 <- read.xlsx(paste0(output_dir, "output/2030_snake_forecast.xlsx"))
df3 <- read.xlsx(paste0(output_dir, "output/2040_snake_forecast.xlsx"))
df4 <- read.xlsx(paste0(output_dir, "output/2050_snake_forecast.xlsx"))
...
write.xlsx(merge_years, paste0(output_dir, "output/snake_forecast_2020_2030_2040_2050.xlsx"))
...
## Age-standardized rates by region - internal only until forecasting manuscript out
gg_prev_age_std <- ggplot(data=region_agestd, aes(x=year_id, y=mean, group=region_name, color=region_name, fill=region_name)) +
  ...
  theme(legend.position = "bottom") +
  guides(color=guide_legend(ncol=7)) +
  ggtitle("Forecast to 2050 of envenomation, age-standardized prevalence, both sexes") +
