...
assemble_prior<-function(data, rmses, cov_list, data_transform,
  ...
  if(F){
    plot_mods <- T
    plot_mods_path <- paste0("[DIRECTORY]")
    weight_col <- "out_rmse"
  }
  ...
  ################ SCRIPTS ################
  #########################################
  stgpr_central<-"[DIRECTORY]"
  central<-paste0("[DIRECTORY]")
  ...
  # get subnat locs for dropping from predictions (get dropped from data later)
  ...
  for(sexchar in sex_list){
    ...
    if(plot_mods==T){
      ...
      for(loc in unique(sqr.s$location_id)){
        
        args<-c(loc, sexchar, plot_mods_path)
        arg_names<-c("loc", "sexchar", "plot_mods_path")
        arg_set<-paste0("--", paste(arg_names, args), collapse=" ")
        
        command<-paste0("qsub -q all.q -l fthread=1 -l h_rt=00:05:00 -l m_mem_free=2G", " -P ", proj, " -N ", paste0("average_", sexchar, "_", loc),
                        " -o [DIRECTORY]/", username, "/output -e [DIRECTORY]/", username, "/errors",
                        " [DIRECTORY]/plot_ave_shell.sh ",
                        loc, " ", sexchar, " ", plot_mods_path, " ", age_trend)
        system(command)
