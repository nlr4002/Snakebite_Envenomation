...
launch_prior <- function(me, decomp_step, crosswalk_version_id, test_mods=TRUE, average=TRUE,
                         ...
                         custom_covs=NULL, random_effects=NULL, username=username, by_sex, path_to_data){
  
  #####################################
  ############### PATHS ###############
  #####################################
  output_dir <- paste0("[DIRECTORY]/",me,"/rmse/")
  message(paste0("Creating output directory at: ",output_dir))
  dir.create(output_dir, recursive=TRUE)
  
  rmse_output <- paste0("[DIRECTORY]/",me,"/rmse/", me, "_prior_test_yes_prior_sign", date, ".csv")  # where the object returned by test_prior is saved
  data_output <- paste0("[DIRECTORY]/",me,"/", me,"_best_data.csv")
  ensemble_output <- paste0("[DIRECTORY]/",me,"/", me,"_custom_prior_", date, ".csv")
  plot_mods_path <- paste0("[DIRECTORY]/", me, "/") 
  
  #######################################
  ############### SCRIPTS ###############
  #######################################
  source("[DIRECTORY]/get_ids.R")
  source("[DIRECTORY]/st_gpr_test_prior.R")
  source("[DIRECTORY]/assemble_prior.R")
  source("[DIRECTORY]/bind_covariates.R")
  source("[DIRECTORY]/linear_oos/helpers.R")
  ...
  if(average==T){
    ...
    setnames(ave_models, "ave_result", "cv_custom_stage1")
  ...
  if(plot_betas==T){
    source("[DIRECTORY]/plot_prior_betas.R")
  }
